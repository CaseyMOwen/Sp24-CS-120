...
<div class="items content-wrapper">
    ...
    <div class="items-wrapper">
        <?php foreach ($items as $item): ?>
            <div class="item">
                <a href="/cs120/FinalProject/swaparoo/items/items/?item=<?=$item['item_id']?>">
                ...
                <form class="add-to-cart-form" action="/cs120/FinalProject/swaparoo/cart/addtocart.php" method="post">
    ...
    <div class="buttons">
        <div>
            <!-- <a href="/cs120/FinalProject/swaparoo/cart/">Go to Pouch</a> -->
        </div>
        <div>
            ...
            <?php if ($current_page > 1): ?>
                <a href="/cs120/FinalProject/swaparoo/items/?p=<?=$current_page-1?>&sort_by=<?=$sort_by?>&order=<?=$order?><?=$urlParamParts?>">Prev</a>
            <?php else: ?>
            ...
            <?php if ($total_items > ($current_page * $num_items_on_each_page) - $num_items_on_each_page + count($items)): ?>
                <a href="/cs120/FinalProject/swaparoo/items/?p=<?=$current_page+1?>&sort_by=<?=$sort_by?>&order=<?=$order?><?=$urlParamParts?>">Next</a>
            <?php else: ?>
...
<script>
    ...
    function updateUrlWithSortOrder(sort_by, order, page = null) {
        const url = `/cs120/FinalProject/swaparoo/items/?p=${page || <?=$current_page?>}&sort_by=${sort_by}&order=${order}<?=$urlParamParts?>`;
    ...
    function addToCart(formData) {
        ...
        xhr.open('POST', '/cs120/FinalProject/swaparoo/cart/', true);
        xhr.onload = function () {
            if (xhr.status === 200) {
    ...
    function setupFilterFormSubmitListener() {
        ...
        filterForm.addEventListener('submit', function(event) {
            ...
            const url = `/cs120/FinalProject/swaparoo/items/?sort_by=${sort_by}&order=${order}&conditionFilter=${conditionFilters.join(',')}&authorFilter=${selectedAuthors.join(',')}&availabilityFilter=${availabilityFilters.join(',')}&minCreditValueFilter=${minCreditValue}&maxCreditValueFilter=${maxCreditValue}`;
            window.location.href = url;
        });
    }
